...
export async function GET(req: Request) {
  try {
    ...
    const collection = db.collection("data quality"); //change to data quality
...
const calculateAggregateScores = (documents: any[]) => {
  ...
  const MAX_SCORE = 6; // Maximum possible score for each area
...
const parseResponses = (responses: Record<string, string>) => {
  ...
  const allowedQuestions = ["4a", "4ai", "4aii", "4b", "4bi", "4bii"];
